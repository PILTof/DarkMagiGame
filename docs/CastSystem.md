...
- `src/systems/BoundsSystem.ts` создаёт баунды юнита, кладёт в `userData.modifiers` модификатор `health_points` и уже использует его для точечного урона.
...
5. Найти юнитов, стоящих в этих клетках. Для MVP достаточно сравнивать `tileMap.worldToGrid(unit.position)` с множеством клеток AoE; позднее можно добавить геометрическое пересечение.
6. Для каждого юнита определить баунд, попавший под эффект, и прочитать его модификаторы.
7. Рассчитать урон, применить `unit.takeDamage(damage)`, сохранить результат hit.
...
Метод должен работать с метаданными баунда (`entityId`, относительная клетка, `modifiers`), а не запускать raycast от камеры. Для каждого задетого юнита выбирается только один баунд; это исключает двойной урон, если в области находится несколько мешей одного юнита.

### Расчёт урона MVP

Текущая точечная атака в `CombatSystem` использует:
...
Для первого AoE-скилла использовать ту же семантику для **каждого** задетого юнита:

```ts
const boundModifier = bound?.modifiers.health_points ?? 1;
const damage = boundModifier * 6;
unit.takeDamage(damage);
```

Значение `6` необходимо вынести в определение навыка (`baseDamage`), а не хранить в `CastSystem`. Позже формула расширяется до `baseDamage * boundModifier * casterPowerMultiplier`; округление, критический урон, сопротивления и friendly fire должны быть правилами навыка, а не UI-логикой.
...
6. **Bound-query и урон:** предоставить выбор баунда по клетке, рассчитать урон от `health_points`, вызвать `Unit.takeDamage` ровно один раз на юнита.
...
- Каждый юнит в AoE получает урон один раз, рассчитанный по `health_points` релевантного баунда; юниты вне AoE не меняют HP.
