...
export class MapLoader {
  async load(path: string): Promise<MapData> {
  ...
  private validate(data: unknown): asserts data is MapData {
    ...
    for (const [rowIndex, row] of map.terrain.rows.entries()) {
      if (typeof row !== "string" || row.length !== map.width) {
        throw new Error(
          `terrain row ${rowIndex} must be a string of length ${map.width}`,
        );
      }
    ...
    for (const object of map.objects) {
      this.assertInBounds(object.q, object.r, map.width, map.height, "object");
    }

    for (const spawn of map.spawns) {
      this.assertInBounds(spawn.q, spawn.r, map.width, map.height, "spawn");
    }
  }
